...
from openrlhf.models.utils import compute_approx_kl, masked_mean, compute_reward_dense
from openrlhf.utils.distributed_util import torch_dist_barrier_and_cuda_sync
from openrlhf.utils.logging_utils import init_logger
from openrlhf.utils.remote_dense_srm_utils import remote_rm_fn_ray
...
def right_strip_pad_token(text, eos_token, sentence_token):
    eos_token_escaped = re.escape(eos_token)
    sentence_token_escaped = re.escape(sentence_token)
    pattern = f"({eos_token_escaped}|{sentence_token_escaped})+$"
    text = re.sub(pattern, "", text)
    return text
...
class RemoteExperienceMaker(BaseExperienceMaker):
    ...
    def _map_sentence_rewards_to_action_mask(self, samples, sentence_rewards):
        """
        Map sentence-level rewards to the corresponding tokens in the action_mask.
        """
        # samples: current batch of Samples
        # sentence_rewards: list of tensor, each tensor shape: [num_sentences]
        device = samples.sequences.device
        action_mask = samples.action_mask
        rewards = torch.zeros_like(action_mask, dtype=torch.float, device=device)
        batch_size = action_mask.size(0)
        response_len = samples.action_mask.shape[-1]
        all_texts = self.tokenizer.batch_decode(samples.sequences, skip_special_tokens=True)
        prompts_texts = self.tokenizer.batch_decode(samples.sequences[:, :-response_len], skip_special_tokens=True)
        response_texts = self.tokenizer.batch_decode(samples.sequences[:, -response_len:], skip_special_tokens=True)
        strip_response_texts = []
        for item in response_texts:
            item = strip_sequence(item, self.tokenizer.pad_token, self.tokenizer.eos_token)
            strip_response_texts.append(item)
        split_answers = self.spliter.split_texts(strip_response_texts)
        processed_texts = [prompts_texts[i] + self.spliter._end_of_sentence_token + split_answers[i] for i in range(len(samples.labels))]

        for idx in range(batch_size):
            all_texts[idx] = right_strip_pad_token(all_texts[idx], self.tokenizer.eos_token, self.spliter._end_of_sentence_token) + self.tokenizer.eos_token
            assert processed_texts[idx].endswith(self.spliter._end_of_sentence_token)
            processed_texts[idx] = right_strip_pad_token(processed_texts[idx], self.tokenizer.eos_token, self.spliter._end_of_sentence_token) + self.tokenizer.eos_token + self.spliter._end_of_sentence_token
            split_mask, _ = self.spliter.find_split_position(all_texts[idx], processed_texts[idx], self.spliter._end_of_sentence_token)
            split_mask = split_mask[torch.where(split_mask == 1)[0][0] + 1 : 1024] # skip the prompt mask, also no to exceed the max length
            if split_mask[-1] != 1:
                split_mask[-1] = 1 # compensate for the last token when exceed the max length
            # Get sentence rewards for response only
            sent_reward = torch.tensor(sentence_rewards[idx][1:]).to(device)
            num_sentences = split_mask.sum()
            if num_sentences != len(sent_reward):
                print(f"Sentence count mismatch: {num_sentences} != {len(sent_reward)}")
                print(f"all_texts[idx]: {repr(all_texts[idx])}")
                print(f"processed_texts[idx]: {repr(processed_texts[idx])}")
                print(f"split_mask: {split_mask}")
                print(f"sent_reward: {sent_reward}")
                print(f"idx: {idx}")
                continue
            # Calculate sentence boundaries
            sentence_ends = torch.where(split_mask == 1)[0].to(device)
            # Distribute rewards across tokens
            for end, reward in zip(sentence_ends.long(), sent_reward):
                end = min(end, rewards.shape[1] - 1)
                rewards[idx, end] = reward
    ...
    def make_experience(self, rollout_samples: Samples) -> List[Experience]:
        ...
        experiences = []
        # TODO: ZXQ add sentence split
        response_len = rollout_samples.action_mask.shape[-1]
        prompts_texts = self.tokenizer.batch_decode(rollout_samples.sequences[:, :-response_len], skip_special_tokens=True)
        response_texts = self.tokenizer.batch_decode(rollout_samples.sequences[:, -response_len:], skip_special_tokens=True)
        ...
        # get mask from here?, write a function to get mask from split_answers
        ...
        if not self.remote_rm_url:
            ...
            if self.strategy.ring_attn_group is None or self.strategy.ring_attn_rank == 0:
                ...
                if self.custom_reward_func:
                    ...
                    # 这边返回的变成了一个list of list，因为返回的是每一个样本的每一个子句的回答
                    sentence_rewards = remote_rm_fn_ray.remote(
                        rm, 
                        raw_prompts=labels_list,
                        answers=split_answers,
                    ) 
                r_refs.append(sentence_rewards)
            else:
        ...
        for i, (samples, action_log_probs, base_action_log_probs, value) in enumerate(
            ...
            sentence_rewards = rewards_list[start_idx:start_idx + action_log_probs.shape[0]]
            start_idx += action_log_probs.shape[0]
            if base_action_log_probs is not None:
                base_action_log_probs = base_action_log_probs.to(device)
            if value is not None:
                value = value.to(device)
            assert isinstance(sentence_rewards, list) # list of list 
            ...
            for s in sentence_rewards:
                r = torch.tensor(s).sum(dim=-1).to(device)
                rewards.append(r)
            sentence_rewards = self._map_sentence_rewards_to_action_mask(samples, sentence_rewards)
            ...
            info = {
                "kl": kl_mean,
                "reward": torch.stack(rewards, dim=0).to(device),
                "response_length": samples.response_length,
                "total_length": samples.total_length,
            }
            
            if sentence_rewards is not None:
                info["sentence_rewards"] = sentence_rewards
    ...
    def compute_advantages_and_returns(
        ...
        rewards = [experience.info["sentence_rewards"] for experience in experiences]

        # reward shaping
        if args.advantage_estimator == "rloo":
            raise NotImplementedError("RLOO is not implemented")
        elif args.advantage_estimator in ["reinforce_baseline", "dr_grpo"]:
            raise NotImplementedError("Reinforce baseline and Dr. GRPO are not implemented")
        elif args.advantage_estimator == "group_norm":
            raise NotImplementedError("Group norm is not implemented")

        # calculate return and advantages
        for experience, reward in zip(experiences, rewards):
            experience = experience.to_device("cuda")
            reward = reward.to(device="cuda") # tensor as the same shape of action_mask
            reward = compute_reward_dense(
                reward,
                self.kl_ctl.value,
                experience.kl,
