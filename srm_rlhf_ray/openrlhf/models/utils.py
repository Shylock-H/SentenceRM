...
def compute_reward_dense(
    ...
    """Compute dense rewards by combining token-level rewards with KL penalty.
    
    Args:
        r: Token-level rewards with shape (B, seq_len)
        kl_coef: KL penalty coefficient 
        kl: KL divergence values, either tensor or list of tensors
        action_mask: Optional mask for valid actions
        reward_clip_range: Optional tuple of (min, max) to clip rewards
        
    Returns:
        Combined rewards with same shape as input rewards
    """
    if kl_coef <= 0.0:
        kl_coef = 0.0
    ...
    if action_mask is not None:
        ...
        kl_reward = -kl_coef * kl
        # Only apply rewards where action_mask is True
        reward = r * action_mask + kl_reward
    else:
        raise NotImplementedError("Dense reward is not implemented")
