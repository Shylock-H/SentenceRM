...
def batch_generate_vllm(args):
    ...
    prompts_dataset = PromptDataset(
        prompts_data, tokenizer, dummy_strategy, input_template=args.input_template
    )
    prompts = list(prompts_dataset)
    original_prompts = [item[2] for item in prompts]
    ...
    # 写文件，格式为二维list
    import json
    with open(args.output_path, mode="w") as writer:
        writer.write("[\n")
        for i, group in enumerate(grouped_outputs):
            if args.best_of_n > 1:
                writer.write("[\n")
...
def batch_generate(args):
    ...
    for _, prompts, origian_prompts in pbar:
        # Conditional SFT inference
        if args.enable_csft:
            for i in range(len(prompts)):
                prompts[i] += args.csft_prompt.strip() + " "
        ...
        for _ in range(N):
            ...
            for origian_prompt, prompt, output in zip(origian_prompts, prompts, outputs):
                output = strip_sequence(output, tokenizer.pad_token, tokenizer.eos_token)
                output = output[len(prompt) :]
                output_dataset.append(
                    {
                        "instruction": origian_prompt,
                        "output": output,
                        "generator": model_name,
                        "dataset": args.dataset,
                    }
    ...
    with open(args.output_path + str(strategy.get_rank()), mode="w") as writer:
        writer.write("[\n")
        for i, item in enumerate(output_dataset):
            writer.write(json.dumps(item))
            if i < len(output_dataset) - 1:
                writer.write(",\n")
        writer.write("\n]")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--dataset_split", type=str, default="eval", help="dataset split")
