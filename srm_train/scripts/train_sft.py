...
from utils.util import get_strategy, get_tokenizer, get_actor_model, add_tokens
...
def parse_args():
    ...
    ## data process
    parser.add_argument(
        "--skip_ENS_token", action="store_true", help = 'Calculate attention score for <END> token'
    )
...
def main():
    ...
    actor_model = get_actor_model(args.model_name_or_path, strategy, tokenizer)
    if args.gradient_checkpointing:
        actor_model.gradient_checkpointing_enable()

    # prepare dataset
    kwargs = dict(skip_ENS_token = args.skip_ENS_token)
    train_dataloader, eval_dataloader = strategy.setup_dataloader(tokenizer, dataset_type = 'sft', **kwargs)
    optimizer = strategy.create_optimizer(actor_model, **vars(args))
    ...
    actor_model, optimizer, lr_scheduler = strategy.ds_init_train_model(actor_model, optimizer, lr_scheduler, ds_config)
    trainer = SFTTrainer(
        model = actor_model,
        strategy = strategy,
        optimizer = optimizer,
