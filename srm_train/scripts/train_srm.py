...
from utils.util import (
    ...
    get_tokenizer, 
    # get_reward_model,
    get_sentence_reward_model,
    add_tokens
)
from utils.data.spliter import Spliter
from utils.global_utils import ENS_TOKEN
...
def parse_args():
    ...
    ## For Sentence Reward Model
    parser.add_argument("--reward_embed_dim", type = int, default = 128)
    parser.add_argument("--penalty_coef", type = float, default = 0.0)
    parser.add_argument("--causal_sentence_mask", action = "store_true")
...
def main():
    ...
    # we should add tokens first, then initialize model
    tokenizer = add_tokens(tokenizer, ENS_TOKEN)

    # rm_model = get_reward_model(args.model_name_or_path, strategy, tokenizer)
    rm_model = get_sentence_reward_model(
        args.model_name_or_path, 
        strategy, 
        tokenizer, 
        reward_embed_dim = args.reward_embed_dim,
        causal_sentence_mask = args.causal_sentence_mask,
    ...
    # prepare spliter
    spliter = Spliter(tokenizer, end_of_conversation_token = tokenizer.eos_token, end_of_sentence_token = ENS_TOKEN)
    spliter.to(torch.cuda.current_device())
    kwargs = dict(spliter = spliter)
    # prepare dataset
    train_dataloader, eval_dataloader = strategy.setup_dataloader(tokenizer, dataset_type = 'sentence-reward', **kwargs)
    # move spliter to cpu to reduce memory cost
    optimizer = strategy.create_optimizer(rm_model, **vars(args))
    # specifical kwargs
    if args.lr_scheduler_type == 'cosine_with_min_lr':
        lr_kwargs = dict(min_lr = args.learning_rate * 0.1)
    else:
        lr_kwargs = None
    lr_scheduler = get_scheduler(
        ...
        scheduler_specific_kwargs = lr_kwargs
    )
