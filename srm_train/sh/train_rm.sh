...
if [ "$OUTPUT" == "" ]; then
    ...
    OUTPUT="./log/reward_model-${MODEL_NAME/'/'/_}-$TIME_STEP-$SEED"
fi
mkdir -p $OUTPUT

deepspeed --master_port 12335 --include localhost:4,5,6,7 scripts/train_rm.py \
   --data_path $DATA_PATH \
...
bash sh/run_bon.sh $OUTPUT > $OUTPUT/bon.log