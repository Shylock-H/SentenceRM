...
from utils.global_utils import SAT_MODEL, ENS_TOKEN, SPLIT_THRESHOLD
from utils.util import convert_token_to_id
...
class Spliter:
    ...
    def __init__(
        ...
        self.model = SaT(spliter_config.model_name)
    ...
    def tokenization(
        ...
        for text in texts: 
            ...
            text = text.rstrip(self._end_of_sentence_token) + self._end_of_text_token + self._end_of_sentence_token
    ...
    def tokenization_wo_end(
        ...
        for text, processed_text in zip(src_texts, processed_texts): 
            ...
            assert processed_text.endswith(self._end_of_sentence_token), print('#' * 30 + repr(processed_text) + '#' * 30)
            ...
            attention_mask[0][eos_indice] = True
    ...
    def find_split_position(self, src_text: str, split_text: str, split_token: str = '<END>'):
        ...
        # assert len(split_indices) == len(split_sentences), f'{len(split_indices)} != {len(split_sentences)}'
